...
Player* GetPlayerPtr(GwClient *client)
{
    if (!client->ingame) {
        return NULL;
    }

    if (client->world.players.size <= client->world.player_id)
        return NULL;
    Player *player = client->world.players.data[client->world.player_id];
...
Party* GetPlayerPartyPtr(GwClient *client)
{
    Player *player;
    if ((player = GetPlayerPtr(client)) == NULL)
        return NULL;
    if (!player->party_id)
        return NULL;
    Party *party;
    if ((party = get_party_safe(client, player->party_id)) == NULL) {
        return NULL;
    }
    return party;
}

Guild* GetPlayerGuildPtr(GwClient *client)
{
    Player *player;
    if ((player = GetPlayerPtr(client)) == NULL)
        return NULL;
    if (!player->guild_id)
        return NULL;
    Guild *guild;
    if ((guild = get_guild_safe(client, player->guild_id)) == NULL) {
        return NULL;
    }
    return guild;
}
...
HQAPI bool GetPlayerIsPartyLeader(uint32_t player_id)
{
    ...
    thread_mutex_lock(&client->mutex);
    Party *party;
    if ((party = GetPlayerPartyPtr(client)) == NULL) {
        goto leave;
    }
...
HQAPI AgentId GetMyAgentId(void)
{
    ...
    if (!client->ingame)
        return 0;
    return client->world.player_agent_id;
}
...
HQAPI uint32_t GetMyPlayerId(void)
{
    ...
    if ((player = GetPlayerPtr(client)) == NULL)
        goto leave;
    player_id = client->world.player_id;
leave:
    thread_mutex_unlock(&client->mutex);
    return player_id;
}
...
HQAPI size_t GetGuildMembers(ApiGuildMember *buffer, size_t length)
{
    ...
    thread_mutex_lock(&client->mutex);
    Guild *guild;
    if ((guild = GetPlayerGuildPtr(client)) == NULL)
        goto leave;
...
HQAPI bool GetAgentOfPlayer(ApiAgent *api_agent, uint32_t player_id)
{
    ...
    thread_mutex_lock(&client->mutex);
    if (!player_id) {
        player_id = client->world.player_agent_id;
    }
...
HQAPI void GetSkillbar(uint32_t *skills, AgentId agent_id)
{
    ...
    if (!(client->ingame && client->world.hash))
        goto leave;
    if (agent_id == 0)
        agent_id = client->world.player_agent_id;
...
HQAPI bool GetSkillCasting(size_t pos, AgentId *target_id)
{
    ...
    if (!(client->ingame && client->world.hash))
        goto leave;
    Skillbar *skillbar = get_skillbar_safe(client, client->world.player_agent_id);
    if (!skillbar)
        goto leave;
...
HQAPI uint32_t GetPartyLeader(uint32_t party_id)
{
    ...
    if (party_id) {
        ...
        party = GetPlayerPartyPtr(client);
    }
    if (!party) goto leave;
    ArrayPartyPlayer players = party->players;
    if (players.size > 0)
