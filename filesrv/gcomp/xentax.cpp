...
public:
    ...
    void ConsumeBits(unsigned int bitsCount)
    {
        assert(bitsCount <= (sizeof(Buf1) * 8));
        Buf1 = (Buf2 >> (32 - bitsCount)) | (Buf1 << bitsCount);

        if (BitsAvailable < bitsCount)
        {
            if (CurInputData == InputDataEnd)
            {
                BitsAvailable = 0;
                Buf2 = 0;
            }
            else
            {
                ...
                unsigned int NewBitsAvailable = (BitsAvailable + 32) - bitsCount;

                Buf1 |= (Buf2 >> NewBitsAvailable);
                Buf2 = Buf2 << (bitsCount - BitsAvailable);
                BitsAvailable = NewBitsAvailable;
            }
        }
        else
        {
            BitsAvailable -= bitsCount;
            Buf2 = Buf2 << bitsCount;
        }
    }

    unsigned int GetBits(unsigned int bitsCount)
    {
        assert(bitsCount <= (sizeof(Buf1) * 8));
        return Buf1 >> (32 - bitsCount);
    }

    unsigned int GetNextCode(HuffmanData *HData)
    {
        // We always read the next 8 bits, but they may not all be used.
        // So, we don't consume right away.
        unsigned int nextBits = GetBits(8);
        ...
        if (encodingLen == 0xFFFFFFFF) {
            //unsigned int nextWord = GetBits(32);

            int idx;
            for (idx = 0; idx < ARRAY_SIZE(HData->LargeEncodingTable); ++idx) {
                if (HData->LargeEncodingTable[idx].FirstEncoding <= Buf1)
                    break;
            }
            ...
            unsigned int groupIdx = (Buf1 - HData->LargeEncodingTable[idx].FirstEncoding) >> (32 - encodingLen);
        ...
        assert(encodingLen != 0 && "Not sure if that's can be asserted");

        ConsumeBits(encodingLen);
        return code;
    }
    ...
    unsigned char* DecompressFile(unsigned char *Input, int InputSize, int OutputSize)
    {
        ...
        CurInputData = (unsigned int *)Input;
        InputDataEnd = (unsigned int *)(Input + InputSize);
        ...
        ConsumeBits(4); // Ignore the first 4 bits (is it always 0b1111 anyway?)
        ...
        int first4Bits = GetBits(4);
        ConsumeBits(4);

        while (ptrOutputEnd != ptrOutput)
        {
            if (!SetupNodesandTree(LiteralCodeTable))
            {
                if (LiteralCodeTable.CodesForLargeEncoding)
                {
                    delete[] LiteralCodeTable.CodesForLargeEncoding;
                }
                delete[] Output;
                return 0;
            }

            if (!SetupNodesandTree(DistanceCodeTable))
            {
                if (DistanceCodeTable.CodesForLargeEncoding)
                {
                    delete[] DistanceCodeTable.CodesForLargeEncoding;
                }
                delete[] Output;
                return 0;
            }
            ...
            unsigned int blockInKbs = GetBits(4) + 1;
            ...
            for (unsigned int loopCount = 0; loopCount < blockSize; ++loopCount)
            {
                ...
                code = GetNextCode(&LiteralCodeTable);
                ...
                else
                {
                    ...
                    code = GetNextCode(&DistanceCodeTable);
                    ...
                    if (len)
                    {
                        backtrack = backtrack | GetBits(len);
                        ConsumeBits(len);
                    }

                    assert((ptrdiff_t)backtrack < (ptrdiff_t)(ptrOutput - Output));
                    assert((ptrdiff_t)backtrackCount < (ptrdiff_t)(ptrOutputEnd - Output));
    ...
    bool SetupNodesandTree(HuffmanData& HData)
    {
        ...
        for (unsigned int CurSymbol = SymbolsCount - 1; CurSymbol < SymbolsCount;)
        {
            ...
            for (idx = 0; ARRAY_SIZE(Table1); ++idx) {
                if (Table1[idx].FirstEncoding <= Buf1)
                    break;
            }
            ...
            unsigned int offset = (Buf1 - Table1[idx].FirstEncoding) >> (32 - bitCount);
        ...
        unsigned int nextBitEncoding = 1;
        for (unsigned int encodingLen = 1; encodingLen < 9; ++encodingLen)
        {
        ...
        if (SymbolsInHuffmanTable < TotalSymbolCount)
        {
            ...
            HData.CodesForLargeEncoding = new unsigned int[NumberOfRemainingCode];
...
{
    Decompress d;
    *output = d.DecompressFile(input, insize, *outsize);
}
